...
const config = {
    BOT: '𝐙𝐄𝐔𝐒⚡', 
    instagram: 'zultra_edits?r=nametag', 
    nomer: 'wa.me/556993899391',
    youtube: 'https://bit.ly/3pn6lpK', 
...
client.on('group-participants-update', async (anu) => {
	...
	client.on('message-new', async (mek) => {
		try {
			...
			mess = {
				wait: '❬❗❭AGUARDE ESTOU PROCURANDO❬❗❭',
				success: '️❬ ✔ ❭ ACHEIII 🙂',
				error: {
					stick: 'Falha :(',
					Iv: 'Link inválido desculpa🙁'
				},
				only: {
					group: '❬❗❭ COMANDO SÓ PODE SER USADO EM GRUPOS❬❗❭ ',
					ownerG: '❬❗❭ COMANDO EXCLUSIVO PARA O ZEUS.❬❗❭ ',
					ownerB: '❬❗❭  COMANDO EXCLUSIVO PARA O ZEUS.❬❗❭  ',
					admin: ' ❬ ⚠️ ❭ ESSE COMANDO É PERMITIDO APENAS PARA O ADMIN ❬ ⚠️ ❭ ',
					Badmin: '❬❗❭ EU PRECISO SER ADMIN MANO '
				}
			}

			const botNumber = client.user.jid
			const ownerNumber = ["556993899391@s.whatsapp.net"] 
...
case 'lofi':
                ...
                case 'reportarbug':
                    ...
                    reply('BUG REPORTADO SE FOR BEIT EU TE COMO😛✌️')
                    break
					case 'marcar':
					members_id = []
				...
				case 'ytbaixa':
					...
					break
                case 'logo3d':
              	    if (args.length < 1) return reply('qual txt linda??')
                    teks = `${body.slice(8)}`
                    if (teks.length > 10) return client.sendMessage(from, 'maximo 10 letras garai', text, {quoted: mek})
			...
			case 'mapa': 
				...
				case 'txtdafoto':
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						reply('Escolha a foto para pegar os textod dela ${prefix} >-<')
					}
					break
				case 'stiker': 
				case 'sticker':
				...
				case 'audio':
					...
					if (args.length < 2) return client.sendMessage(from, 'qual texto tu quer lindaw?', text, {quoted: mek})
					dtt = body.slice(9)
					ranm = getRandom('.mp3')
					rano = getRandom('.ogg')
					dtt.length > 300
					? reply('affs olha o tamanho desse texto😤')
					: gtts.save(ranm, dtt, function() {
						exec(`ffmpeg -i ${ranm} -ar 48000 -vn -c:a libopus ${rano}`, (err) => {
							fs.unlinkSync(ranm)
							buff = fs.readFileSync(rano)
							if (err) return reply('falhei, me desculpa por ser inutil')
				...
				case 'sigla':
					...
					break 	
				case 'hilih': 
					if (args.length < 1) return reply('qual texto deseja lindu?')
				...
				case 'tiktokperfil':
					try {
						if (args.length < 1) return client.sendMessage(from, 'qual nome da pessoa garai??', text, {quoted: mek})
				...
				case 'clearall':
				case 'limpar':
					if (!isOwner) return reply(' quem e tu man?')
					anu = await client.chats.all()
					client.setMaxListeners(25)
					for (let _ of anu) {
						client.deleteChat(_.jid)
					}
					reply('esta limpo chefia  :)')
					break
			       case 'bloquear':
					if (!isGroup) return reply(mess.only.group)
					if (!isOwner) return reply(mess.only.ownerB)
				...
				case 'adicionar':
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (args.length < 1) return reply('Qual o número da pessoa burro??')
					if (args[0].startsWith('08')) return reply('qual o ddd do pais?? ')
					try {
       ...
       client.sendMessage(from, 'Este é o número do meu criador ^-^ ',MessageType.text, { quoted: mek} )
           ...
           case 'mudarnome':
                ...
                client.sendMessage(from, 'descriçao alterada com sucesso', text, {quoted: mek})
                break
           case 'demote':
           case 'rebaixar':
					if (!isGroup) return reply(mess.only.group)
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('marca o adm que deseja rebaixar')
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = ''
						for (let _ of mentioned) {
							teks += `nao fez seu papel de adm e virou membro comum 😛✌️ :\n`
							teks += `@_.split('@')[0]`
						}
						mentions(teks, mentioned, true)
						client.groupDemoteAdmin(from, mentioned)
					} else {
						mentions(`nao mamou o bonde direitinho virou membro comum @${mentioned[0].split('@')[0]} seu gay`, mentioned, true)
						client.groupDemoteAdmin(from, mentioned)
					}
					break
				case 'promote':
				case 'promover':
					...
					if (mentioned.length > 1) {
						teks = ''
						for (let _ of mentioned) {
							teks += `novo admin entao respeitem taok :\n`
							teks += `@_.split('@')[0]`
						}
						mentions(teks, mentioned, true)
						client.groupMakeAdmin(from, mentioned)
					} else {
						mentions(`novo adm eitaaa @${mentioned[0].split('@')[0]} esse é brabo einn`, mentioned, true)
						client.groupMakeAdmin(from, mentioned)
					}
					break	
			     	case 'kick':
			...
			case 'remover':
					...
					if (mentioned.length > 1) {
						teks = ''
						for (let _ of mentioned) {
							teks += `removi mermo sou adm😛 :\n`
							teks += `@_.split('@')[0]`
						}
						mentions(teks, mentioned, true)
						client.groupRemove(from, mentioned)
					} else {
						mentions(`removi mermo @${mentioned[0].split('@')[0]} sou adm🏃`, mentioned, true)
						client.groupRemove(from, mentioned)
					}
					break
				case 'listadmin':
				...
				case 'toimg':
				case 'converter':
					if (!isQuotedSticker) return reply('marca o stiker man')
					reply(mess.wait)
					...
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						fs.unlinkSync(media)
						if (err) return reply('erro, tente novamente ^_^')
						buffer = fs.readFileSync(ran)
						client.sendMessage(from, buffer, image, {quoted: mek, caption: 'tchau entao '})
						fs.unlinkSync(ran)
					})
					break
				case 'nsfw':
